...
## 大众点评网(2015/6 ~ 2015/10)
### POI（Point of Interest）与基础内容
####菜单匹配服务
将用户推荐菜与系统商户输入菜单进行匹配，前期使用Python脚本编写算法部分，接收推荐菜与系统菜单组成的CSV输入文件，输出CSV格式的匹配结果，用shell脚本从Hive数据库中拉取数据，使用Linux的Crontab配置每天每一小时执行一次定时任务。

**存在的问题**：此种处理方式为全量更新，当系统菜单数据量越来越大的时候，每小时拉取一次数据都会给Hive数据库带来很大压力，所以后期使用Java语言，采用增量更新的方式进行了重构。

Java重构之后的**增量更新解决方案**：
① 由DBA方负责出一个拉取系统菜单接口，该接口接收一个开始时间，会根据该参数去检索系统菜单表，检索到的结果都是从该开始时间之后插入到表中的数据
② 根据拉取到的系统菜单去增量拉取推荐菜，两张表通过公共的shopId关联
② 开启线程会定期去调用DBA方出的接口，每次调用传递一个参数，该参数是上一次调用该接口的时间，将获取的数据写入推荐菜与系统菜单关系表，初始化为未匹配
③ 算法开始工作，根据同一个shopId，查询到该商户对应的推荐菜与系统菜单，挖掘菜品之间的关系，工作结束后，写入推荐菜与系统菜单关系表，关系有：精确匹配，格式化后匹配，近似匹配
④ 对外暴露服务，供有需求方进行查询，获取到推荐菜与系统菜单之间的关系

**实现的效果**：
只有该服务第一次启动开始工作的时候，会进行全量更新，给Hive数据库带来比较大的压力，之后每次工作都是增量更新基本对数据库无影响，并可以对外提供稳定的服务。
...
* 近似模糊匹配
 ...
 - 计算公共子序列(有序，非连续)
 - 计算公共子串(有序，且连续)
 - 根据评分公式计算相似度并辅之以动态变化的阈值，之所以设置动态阈值基于这样一个事实，不同长度的字符串其某一个字符不同对其影响的大小不同，比如“鸭头”和“鸭脚”，仅有一个字不同，但是表达的完全是不同的意思，再如“江西瓦罐焖蹄饭”和“江西瓦罐蹄膀饭”，虽有两字不同，但是对整体意思并无大碍，所以不同长度的字符串应有不同的阈值，总结短的字符串设置较高的阈值，长的字符串可稍微放宽阈值
...
**实现的效果**
运营人员会不定期对判重服务的结果进行人工抽检，所有的判重结果都在数据库中，让运行人员每次都去写SQL语句查询，或者是写程序去调用判重服务显然是不太现实的，而该客户端只需要接收包含商户信息的输入文件即可，会将判重的结果以CSV或Excel文件的形式返回。运营人员分析判重结果，进而计算判重服务的准确率、召回率以及新增重复率。可以提高运营人员工作效率。

####其他项目
>判重服务的内部分档与业务分档关系配置平台，属于Web平台开发，使用Bootstrap进行页面布局，用JS+Ajax实现同一页面的CRUD操作。

>使用Selenium模拟用户登录支付宝账户，并自动根据交易号或者商户订单号查询交易对方信息，已完成。
